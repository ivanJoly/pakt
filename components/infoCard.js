...
export const InfoCard = ({ children, image, title, description, rotate = 0 }) => {
    return (
        <Flex gap={{ base: 6, md: 12 }} alignItems={'center'}>
            <GlassCard
                isOnHoverActive={!!rotate}
                transform={`translateX(0%) rotate(${rotate}deg)`}
                width={{ base: '75px', sm: '100px' }}
                height={{ base: '75px', sm: '80px' }}
                position={'relative'}
                zIndex={2}
            >
                <Box position={'relative'} overflow={'hidden'} borderRadius={4} display={'flex'} flexDirection={'column'} alignItems={'center'} justifyContent={'center'} padding={4} zIndex={3}>
